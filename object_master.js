...
const multipleofthree = pokemon.filter(p => p.id % 3 === 0);
...
const dualtype = pokemon.filter(p => p.types[1]);
...
const ninetynine = pokemon.filter(p => p.id > 99).map(p => p.name);
// console.log(ninetynine);

// an array with just the names of the pokémon whose only type is poison
const poisontype = pokemon.filter(p => p.types == ("poison")).map(p => p.name);
